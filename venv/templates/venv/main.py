...
def graph():
    ...
    predTemp = json.dumps(predict("°C").tolist())
    predHumidity = json.dumps(predict("%").tolist())
    predLum = json.dumps(predict("lx").tolist())
    predBruit = json.dumps(predict("ppm").tolist())
    predUv = json.dumps(predict("UV index").tolist())
    
    return render_template('graph.html', active_page='Graph', predTemp=predTemp, predHumidity=predHumidity, predLum=predLum, predBruit=predBruit, predUv=predUv)
